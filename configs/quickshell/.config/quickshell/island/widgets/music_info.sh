...
if [ -z "$players" ]; then
    ...
    # Try to find a player that is currently "Playing"
    BEST_PLAYER=""
    while read -r p; do
    ...
    # Fallback to the first player if none are playing
    [ -z "$BEST_PLAYER" ] && BEST_PLAYER=$(echo "$players" | head -n 1)

    # Get metadata for the selected player
    raw=$(playerctl -p "$BEST_PLAYER" metadata --format \
