...
Singleton {
    ...
    property list<QtObject> unreadNotifications: list.filter(n => !n.read)
    ...
    FileView {
        ...
        onLoaded: {
            try {
                ...
                // Set idOffset to avoid collisions with loaded IDs
                if (root.list.length > 0) {
                    const maxId = Math.max(...root.list.map(n => n.notificationId));
