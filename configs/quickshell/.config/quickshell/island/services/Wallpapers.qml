...
Singleton {
    ...
    Process {
        ...
        onExited: {
            Quickshell.execDetached(["swaync-client", "-rs"]);
        }
    }
    ...
    function select(path) {
        ...
        // Sync to swww
        Quickshell.execDetached(["swww", "img", cleanPath, "--transition-type", "grow", "--transition-pos", "top-right"]);
    ...
    function initializeSwww() {
        if (!Config.ready) return;
        const path = Config.options.appearance.background.wallpaperPath;
        const cleanPath = path.toString().startsWith("file://") ? path.toString().substring(7) : path.toString();
        if (cleanPath !== "") {
            Quickshell.execDetached(["swww", "img", cleanPath]);
        }
    }
